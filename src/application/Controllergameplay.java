...
public class Controllergameplay implements Initializable {
	...
	int MAXPOINT=4, counter = 0, incr = 0;
	boolean startrainbow = false;
	boolean startrainbow1 = false;
	private Pane tempObstacle;// will contain the outer frame of the obstacle which would go inside the
								// CurrentObstacle pane
	@FXML
    private AnchorPane BBase;
	@FXML
    private Canvas BallBurstCancas;
	private Pane temp2;
	...
	private void update() throws IOException, NumberFormatException, ClassNotFoundException, InterruptedException {

		// if(start && Integer.valueOf(score.getText())<=MAXPOINT)
		if (start ) 
		{
			...
			if (currentplayer.getshape().getLayoutY() < 300)
			{
				...
				hand.setLayoutY(hand.getLayoutY() + 5);
				logocs.setLayoutY(logocs.getLayoutY() + 5);

				if (hand.getLayoutY() > 750 || logocs.getLayoutY() > 750) {
					// System.out.println("hands and logo are out of picture");
					hand.setVisible(false);
					logocs.setVisible(false);
				}
